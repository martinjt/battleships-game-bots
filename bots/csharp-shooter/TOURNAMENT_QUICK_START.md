...
### Run with Tournament ID

```bash
docker run \
  -e TOURNAMENT_MODE=true \
  -e BOT_NAME=csharp-shooter \
  -e TOURNAMENT_ID=<your-tournament-id> \
  -e GAME_API_URL=https://battleships.devrel.hny.wtf \
  csharp-shooter
```
...
The tournament organizer needs to start games.
...
### Test Without Tournament Mode (Legacy)
